...
impl crate::Instance<super::Api> for Instance {
    ...
    unsafe fn enumerate_adapters(&self) -> Vec<crate::ExposedAdapter<super::Api>> {
        ...
        let gl = match *canvas_guard {
            Some(ref canvas) => {
                let window = web_sys::window().expect("Could not get browser window");
                ...
                if canvas.get_attribute("width").is_none() {
                    canvas
                        .set_attribute(
                            ...
                            &window
                                .inner_width()
                                .expect("Cannot read window width")
                                .as_f64()
                                .unwrap()
                                .to_string(),
                        )
                        .expect("Cannot set canvas width");
                }
                if canvas.get_attribute("height").is_none() {
                    canvas
                        .set_attribute(
                            ...
                            &window
                                .inner_height()
                                .expect("Cannot read window height")
                                .as_f64()
                                .unwrap()
                                .to_string(),
                        )
                        .expect("Cannot set canvas height");
                }
